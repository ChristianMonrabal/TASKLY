document.addEventListener('DOMContentLoaded', function() {
    // Elementos del DOM
    const filtroEstado = document.getElementById('filtroTrabajos');
    ...
    const trabajosContainer = document.querySelector('.row.trabajos-grid');
    ...
    let filtrosActivos = {
        estado: 'todos', // Por defecto mostramos todos
        busqueda: '',
        categorias: [],
    ...
    function aplicarFiltros() {
        ...
        let trabajosFiltrados = Array.from(trabajos).filter(item => {
            // Por defecto, visible
            let visible = true;
        ...
        const trabajosPorPagina = filtrosActivos.itemsPorPagina;
        ...
        // Ocultar todos los trabajos primero
        trabajos.forEach(item => {
            item.style.display = 'none';
        });
        
        // Calcular índices para la paginación
        const inicio = (filtrosActivos.pagina - 1) * trabajosPorPagina;
        const fin = Math.min(inicio + trabajosPorPagina, trabajosFiltrados.length);
        
        // Mostrar solo los trabajos de la página actual
        for (let i = inicio; i < fin; i++) {
            trabajosFiltrados[i].style.display = '';
        }
        ...
        if (noResultados) {
            ...
            if (trabajosFiltrados.length === 0 && hayFiltros) {
    ...
    // Aplicar filtros iniciales (por defecto mostrar activos)
    aplicarFiltros();
});